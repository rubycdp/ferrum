...
module Ferrum
  describe Browser::API::Header do
    ...
    it "adds new headers" do
      browser.headers = { "User-Agent" => "Browser", "DV" => "hello" }
      browser.add_headers("User-Agent" => "Super Browser", "Appended" => "true")
      browser.goto("/ferrum/headers")
    ...
    it "sets headers on the initial request for referer only" do
      browser.headers = { "PermanentA" => "a" }
      browser.add_headers("PermanentB" => "b")
      browser.add_header({ "Referer" => "http://google.com" }, permanent: false)
      browser.add_header({ "TempA" => "a" }, permanent: false) # simply ignored
    ...
    it "keeps added headers on redirects" do
      browser.add_header({ "X-Custom-Header" => "1" }, permanent: false)
    ...
    context "multiple windows", skip: true do
      ...
      it "does not mix temporary headers with permanent ones when propagating to other windows" do
        browser.open_new_window
        browser.add_header("X-Custom-Header", "1", permanent: false)
        browser.add_header("Host", "foo.com")
